#!/usr/bin/env python3
"""
Scrape The Guardian's Taylor Swift music page, store into Postgres guardian table.
...
OUT_JSON     = Path("taylor_swift_guardian_articles.json")
HEADLESS     = True             # change to False to watch browser
WAIT_MS      = 4_000            # page wait-time for JS (ms)
MAX_PAGES    = 1000             # safety stop
# ───────────────────────────────────────────────────────────

# ╭─ 1. Helpers for AWS secrets ────────────────────────────╮
def get_db_creds():
...
def insert_guardian_rows(rows: list[dict]):
    ...
    with psycopg2.connect(**{
        ...
        cur.executemany("""
            INSERT INTO guardian (url, uid, date_timestamp, meta_data, title_context)
            VALUES (%s, %s, %s, %s, %s)
        ...
        conn.commit()
    print(f" Inserted {len(rows)} rows into guardian.")
