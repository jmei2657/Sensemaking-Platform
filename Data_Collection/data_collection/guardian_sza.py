#!/usr/bin/env python3
"""
Scrape The Guardian's SZA music page, store into Postgres guardian_sza table.

Table schema (created if missing):

CREATE TABLE guardian_sza (
...
DB_SECRET    = "DB"
START_URL    = "https://www.theguardian.com/music/sza"
BASE_URL     = "https://www.theguardian.com"
OUT_JSON     = Path("sza_guardian_articles.json")
HEADLESS     = True
WAIT_MS      = 4_000
MAX_PAGES    = 1000
...
def create_guardian_table():
    ...
    with psycopg2.connect(**{
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS guardian_sza (
                url TEXT PRIMARY KEY,
                uid TEXT,
                date_timestamp TIMESTAMP,
                meta_data TEXT,
...
def insert_guardian_rows(rows: list[dict]):
    ...
    with psycopg2.connect(**{
        ...
        cur.executemany("""
            INSERT INTO guardian_sza (url, uid, date_timestamp, meta_data, title_context)
            VALUES (%s, %s, %s, %s, %s)
...
def scrape_article(page, url: str) -> dict:
    ...
    if "sza" not in body_text.lower():
        raise ValueError("Irrelevant article (Sza not mentioned)")
