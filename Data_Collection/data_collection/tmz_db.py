...
url = "https://www.tmz.com/people/taylor-swift/"
now = datetime.now()
scrape_time = now.strftime('%Y-%m-%d %H:%M:%S')
uid = f"tmz_taylor_{now.strftime('%Y%m%d_%H%M')}"
filename = f"{uid}.json"
...
def create_tmz_table():
    ...
    cur = conn.cursor()
    cur.execute("""
        CREATE TABLE IF NOT EXISTS tmz (
            uid TEXT,
            title TEXT,
...
def insert_tmz_articles(articles):
    ...
    cur = conn.cursor()
    insert_query = """
        INSERT INTO tmz (uid, title, published_date, excerpt, scrape_time)
        VALUES (%s, %s, %s, %s, %s);
    """
    for art in articles:
