#!/usr/bin/env python3
"""
Scrape The Guardian's Billie Eilish music page, store into Postgres guardian_billie table.

Table schema (created if missing):

CREATE TABLE guardian_billie (
...
DB_SECRET    = "DB"
START_URL    = "https://www.theguardian.com/music/billie-eilish"
BASE_URL     = "https://www.theguardian.com"
OUT_JSON     = Path("billie_eilish_guardian_articles.json")
HEADLESS     = True
WAIT_MS      = 4_000
MAX_PAGES    = 1000
...
def create_guardian_table():
    ...
    with psycopg2.connect(**{
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS guardian_billie (
                url TEXT PRIMARY KEY,
                uid TEXT,
                date_timestamp TIMESTAMP,
                meta_data TEXT,
...
def insert_guardian_rows(rows: list[dict]):
    ...
    with psycopg2.connect(**{
        ...
        cur.executemany("""
            INSERT INTO guardian_billie (url, uid, date_timestamp, meta_data, title_context)
            VALUES (%s, %s, %s, %s, %s)
        ...
        conn.commit()
    print(f" Inserted {len(rows)} rows into guardian_billie.")
...
def get_article_links(html: str):
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "billie eilish" in text or "billie-eilish" in href:
            if href.startswith("/"):
                href = BASE_URL + href
...
def scrape_article(page, url: str) -> dict:
    ...
    if "billie eilish" not in body_text.lower():
        raise ValueError("Irrelevant article (Billie Eilish not mentioned)")
...
if __name__ == "__main__":
    ...
    with OUT_JSON.open("w", encoding="utf-8") as f:
        json.dump([{**r, "date_timestamp": r["date_timestamp"].isoformat()} for r in rows],
